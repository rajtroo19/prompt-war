...
export default function JournalPage() {
  ...
  useEffect(() => {
    setEntries(getAllJournalEntries().reverse());
    setIsLoaded(true);
  }, []);

  const handleSave = () => {
    if (!content.trim()) return;
    const entry = saveJournalEntry(content.trim(), mood);
  ...
  const handleExportPDF = () => {
    ...
    a.href = url;
    a.download = `mindprep-journal-${new Date().toISOString().split('T')[0]}.txt`;
    a.click();
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                {MOOD_CONFIG.map((m) => (
                  <button
                    key={m.score}
                    onClick={() =>
                      setMood(mood === m.score ? undefined : m.score)
                    }
                    style={{
                      fontSize: '1.5rem',
                      background:
      ...
      {entries.length === 0 ? (
        ...
        >
          ...
          <p style={{ color: '#94A3B8', marginBottom: '1rem' }}>
            Start writing to process your thoughts and track your emotional
            journey.
          </p>
          <button
            className="btn-primary"
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  {entry.mood && (
                    <span style={{ fontSize: '1.2rem' }}>
                      {MOOD_CONFIG[entry.mood - 1]?.emoji}
                    </span>
                  )}
                </div>
                <button
